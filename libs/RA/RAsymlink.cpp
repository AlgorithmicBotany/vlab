...
/******************************************************************************
 *
 * int RA::Symlink( RA_Connection * connection,
...
int RA::Symlink(RA_Connection *connection, const char *src_fname,
                const char *dst_fname) {

  assert(connection != NULL);
  assert(connection->connection_type != RA_NO_CONNECTION);

  debug_printf("RA::Symlink( %s:%s, %s)\n", connection->host_name, src_fname,
               dst_fname);

  // if the connection is local, use symlink(2)
  if (connection->connection_type == RA_LOCAL_CONNECTION) {
    return symlink(src_fname, dst_fname);
  }
  ...
  // send message to the server requesting a SYMLINK operation
  Message request;
  request.code = RA_SYMLINK_REQUEST;
  ...
  Message *response = connection->messagePipe->get_message(RA_SYMLINK_RESPONSE);
  if (response == NULL) {
