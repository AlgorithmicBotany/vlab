...
/*************************************************************************/
/* npars - the number of parameters used by L-system */
primitive_type *AddTriangle(grid_type *grid, float *vertices,
  ...
  if (verbose)
    fprintf(stderr, "Field - adding triangle specified by vertices\n");

  /* it is faster not to use inv matrix */
  SetPrimitive(TRIANGLE, 9, &last_object_array, vertices);
  ...
  *top_mat = *bottom_mat = -1;
  ...
  if (npars >= 0)
    /* next parameter specify material index (top and bottom). */
    if (next_symbol->num_params > npars) {
      *top_mat = next_symbol->params[npars].value - 1;

      if (next_symbol->num_params >= npars + 2)
        *bottom_mat = next_symbol->params[npars].value - 1;
    }
...
primitive_type *AddObject(grid_type *grid, CTURTLE *tu,
  ...
  case 'T': /* triangle */
    if (next_symbol->num_params >= (npars = 2)) {
      ...
      if (next_symbol->params[0].value == 0 ||
          next_symbol->params[1].value == 0)
        /* if one parameter is 0, no object */
        return NULL;
