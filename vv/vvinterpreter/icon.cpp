...
static int      irisError(const char *, char *);
...
void flipIcon( PICINFO * icon)
{
    ...
    newData = (byte *) malloc((size_t) icon-> w * icon-> h * 3);
    for( y = 0; y < icon-> h; y ++)
...
/***************************************************/
FILE *xv_fopen( const char * fname, char * mode)
{
  FILE *fp;
...
/*******************************************/
//static int irisError(const char * name, char * st)
static int irisError(const char *, char *)
{
/*    fprintf( stderr, "Iris.c warning: %s: %s\0", name, st); */
//    name = name;
//    st = st;
    return 0;
}
...
/*************************************************/
int WriteIRIS( FILE * fp, byte * pic, int ptype, int w, int h, byte * rmap, byte * gmap, byte * bmap, int colorstyle)
{
...
static int compressrow( byte * lbuf, byte * rlebuf, int z, int cnt)
{
  ...
  while (iptr<ibufend) {
	...
	*optr++ = *sptr;
    ...
    cc = *iptr;
    iptr += 4;
    while ((iptr<ibufend) && ( (char) (*iptr) == cc))  iptr += 4;
