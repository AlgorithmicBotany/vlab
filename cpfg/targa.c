...
/************************************************************/
static int savebyte(byte c, FILE *fp) { return fwrite(&c, 1, 1, fp); }
...
int saveTGArow(targa_params_type *spec, int y, byte *row) {
  if (spec->current_row != y) {
    Message("Targa error - row number %d doesn't correspond to the "
            "current row %d!\n",
            y, spec->current_row);
    return -1;
  }
  ...
  if ((spec->current_row < 0) || (spec->current_row >= spec->Yres)) {
    Message("Targa error - row is out of the given Yres!\n");
    return -1;
  }
  ...
  Message("Targa - saving of a row not supported for type %d!\n",
          (int)(spec->type + '0'));
...
int saveTGAhead(targa_params_type *spec) {
  ...
  if (spec->fp == NULL) {
    Message("Targa error - file not opened!\n");
    return 0;
  }
  ...
  if (spec->id_field == NULL)
    ...
    savebyte((unsigned char)(strlen(spec->id_field)), spec->fp);

  switch (spec->type) {
  case TGA_MAPPED:
  case TGA_MAPPED_RLE:
    if (spec->colormap == NULL) {
      Message("Targa error -  colormap not specified!\n");
      return 0;
    }
  ...
  case TGA_TRUECOLOR_RLE:
    ...
    break;
  default:
    Message("Targa type %d not supported!\n", (int)('0' + spec->type));
    return 0;
  }
  ...
  case TGA_MAPPED_RLE:
    ...
    savebyte((unsigned char)(spec->first_cmap_entry & 0xff), spec->fp);
    savebyte((unsigned char)(spec->first_cmap_entry >> 8), spec->fp);
    /* (lo-hi) colormap entries */
    savebyte((unsigned char)(spec->num_cmap_entries & 0xff), spec->fp);
    savebyte((unsigned char)(spec->num_cmap_entries >> 8), spec->fp);
...
int loadTGAhead(const char *filename, targa_params_type *spec) {
  ...
  if ((spec->fp = fopen(filename, "r")) == NULL) {
    Message("Targa error - cannot open file %s!\n", filename);
    return 0;
  }

  fread(&skip, 1, 1,
        spec->fp); /* Number of Characters in Identification Field */
  ...
  default:
    Message("Targa error - probably not targa image!\n");
    return 0;
  }
  ...
  fread(&pom, 1, 1, spec->fp);
  if (cmap && (pom != 24)) {
    Message("Targa error - only colormaps with items of size 24 supported!\n");
    return 0;
  }
  ...
  case TGA_TRUECOLOR_RLE:
    if (pom != 24) {
      Message("Targa error - image pixel size 24 expected (is %d)!\n",
              (int)pom);
      return 0;
    }
    break;
  ...
  case TGA_MAPPED_RLE:
    if (pom != 8) {
      Message("Targa error - image pixel size 8 expected (is %d)!\n", (int)pom);
      return 0;
    }
    break;
  }
  ...
  */
  ...
  case TGA_TRUECOLOR_RLE:
    if (pom != 0x20) {
      Message("Targa error - image destriptor 0x20 expected (is %x)\n"
  ...
  if (!cmap)
    ...
    /* only skip for now */
    Message("Targa warning - colormap is ignored.\n");
