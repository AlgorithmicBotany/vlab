...
/************************************************************/
void saveTGAfinish(targa_params_type *spec) { fclose(spec->fp); }
...
int loadTGAhead(char *filename, targa_params_type *spec) {
  ...
  case TGA_TRUECOLOR_RLE:
    if (pom != 0x20) {
      fprintf(stderr,
              "Targa error - image destriptor 0x20 expected (is %x)\n"
...
#ifndef TEST_MALLOC
    if ((spec->id_field = (char *)malloc(skip + 1)) != NULL) {
#else
    if ((spec->id_field =
             test_malloc(skip + 1, "spec->id_field in loadTGAhead")) != NULL) {
#endif
...
/************************************************************/
void loadTGAfinish(targa_params_type *spec) {
  if (spec->id_field != NULL)
    free(spec->id_field);
