...
void PrintMatrix(char *str, mtype *mat) {
  ...
  for (row = 0; row < MAT_SIZE; row++) {
    fprintf(stderr, "     %d row:", row + 1);

    for (col = 0; col < MAT_SIZE; col++)
      fprintf(stderr, "  %g", mat[access(row, col)]);
...
void Normalize(mtype *vec) {
  ...
  if ((len = sqrtf(vec[0] * vec[0] + vec[1] * vec[1] + vec[2] * vec[2])) > 0.0f)
    for (i = 0; i < 3; i++)
      vec[i] /= len;
}
...
void MakeUnitMatrix(mtype *mat) {
  ...
  for (row = 0; row < MAT_SIZE; row++)
    for (col = 0; col < MAT_SIZE; col++)
      mat[access(row, col)] = row == col ? 1.0f : 0.0f;
}
...
float Length(float *v1, float *v2) {
  float lengthsqr = (v2[X] - v1[X]) * (v2[X] - v1[X]) +
                    (v2[Y] - v1[Y]) * (v2[Y] - v1[Y]) +
                    (v2[Z] - v1[Z]) * (v2[Z] - v1[Z]);
...
void Rotate(mtype *vals) {
  ...
  rot[access(2, 2)] = rot[access(1, 1)] = (float)cos(angle);
  rot[access(2, 1)] = -(rot[access(1, 2)] = (float)sin(angle));
...
void Transform3Point(mtype *src, mtype *mat, mtype *dest) {
  ...
  // make homogeneous coordinate from src pt [x y z 1]
  for (i = 0; i < MAT_SIZE - 1; i++)
    pt[i] = src[i];
  pt[MAT_SIZE - 1] = 1.0;

  // multiply pt by matrix
  for (row = 0; row < MAT_SIZE; row++) {
    ptc[row] = 0.0;
    for (i = 0; i < MAT_SIZE; i++)
