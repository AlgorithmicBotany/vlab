...
triangle octahedron[] = {
    {{XPLUS, ZPLUS, YPLUS}, 0.0}, {{YPLUS, ZPLUS, XMIN}, 0.0},
    {{XMIN, ZPLUS, YMIN}, 0.0},   {{YMIN, ZPLUS, XPLUS}, 0.0},
    {{XPLUS, YPLUS, ZMIN}, 0.0},  {{YPLUS, XMIN, ZMIN}, 0.0},
    {{XMIN, YMIN, ZMIN}, 0.0},    {{YMIN, XPLUS, ZMIN}, 0.0}};

/* A unit octahedron */
object oct = {8, &octahedron[0]};
...
void makePolySphere(sphtri *sphere, int resolution) {
  ...
  int ccwflag = 1,  /* Reverse vertex order if true */
      i, j, level,  /* Current subdivision level */
  ...
  for (level = 1; level < maxlevel; level++) {
    ...
    if (new_ == NULL) {
      Message("Out of memory on subdivision level %d\n", level);
      MyExit(1);
    }
    ...
    for (i = 0; i < old->npoly; i++) {
      triangle *oldt = &old->poly[i], *newt = &new_->poly[i * 4];
      point a, b, c;
